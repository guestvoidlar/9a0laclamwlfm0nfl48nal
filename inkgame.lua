...
local function ToggleGodMode(state)
    ...
    if Character then
        ...
        if state then
            Character:PivotTo(currentPivot * CFrame.new(0, 162, 0))
        else
            Character:PivotTo(currentPivot * CFrame.new(0, -162, 0))
        end
    end
...
-- BACKPACK VE TOOL KORUMALI GÖRÜNMEZLİK YAPISI
local originalMaterials = {}
local invisibleAssets = {}
...
local function ToggleInvisibility(state)
    ...
    if state then
        ...
        table.clear(invisibleAssets)
        
        local function applyInvis(obj)
            -- Eşya (Tool) kontrolü: Parça eğer karakterin elindeki veya Backpack içindeki bir Tool'a aitse ASLA dokunma
            local isToolPart = obj:IsDescendantOf(Character:FindFirstChildOfClass("Tool") or workspace) 
                or (Backpack and obj:IsDescendantOf(Backpack))
            
            if isToolPart then return end
            ...
            if obj:IsA("BasePart") then
                ...
                if obj.Name ~= "HumanoidRootPart" then
                    obj.Transparency = 1
                    obj.LocalTransparencyModifier = 0.6 -- Kendimize hayalet efekti
                end
            ...
            elseif obj:IsA("Clothing") or obj:IsA("ShirtGraphic") or obj:IsA("CharacterMesh") then
                obj.Parent = nil
                table.insert(invisibleAssets, obj)
                
            elseif obj:IsA("Accessory") then
                local handle = obj:FindFirstChild("Handle")
                if handle and handle:IsA("BasePart") then
                    if not originalMaterials[handle] then
                        originalMaterials[handle] = {
                            Transparency = handle.Transparency,
                            LocalTransparencyModifier = handle.LocalTransparencyModifier
                        }
                    end
                    handle.Transparency = 1
                    handle.LocalTransparencyModifier = 0.6
                end
            end
        end

        -- Karakter içindeki mevcut parçalara uygula
        for _, part in ipairs(Character:GetDescendants()) do
            applyInvis(part)
        end
        
        -- Karakter dinamik olarak güncellenirse (Örn: Eşya kuşanıp bırakırken) tetiklenir
        invisConnection = Character.DescendantAdded:Connect(function(desc)
            task.wait()
        ...
        for _, asset in ipairs(invisibleAssets) do
            if asset then
...
-- Çanta ve Tool Korumalı Görünmezlik
chGroupBox:AddToggle("InvisibleToggle", {
    Text = "Görünmezlik (Invisible)",
    Default = false,
    Callback = function(State)
