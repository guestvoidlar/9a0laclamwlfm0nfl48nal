...
-- TEK TOGGLE BAĞLANTILI GELİŞMİŞ DESYNC GÖRÜNMEZLİK YAPISI
local FakeBody = nil
local DesyncConnection = nil

local function ToggleDesyncInvisible(state)
    if state then
        -- --- DESYNC AKTİF ETME KISMI ---
        if DesyncConnection then DesyncConnection:Disconnect() end
        
        -- 1. Sahte Bedeni (Fake Body) Dünyaya Bırakıyoruz
        if Character then
            Character.Archivable = true
            FakeBody = Character:Clone()
            Character.Archivable = false
            
            for _, child in pairs(FakeBody:GetChildren()) do
                if child:IsA("BasePart") then
                    child.Transparency = 0 -- Sunucu bunu gerçek karakter sansın
                    child.CanCollide = true
                elseif child:IsA("LocalScript") or child:IsA("Script") then
                    child:Destroy() -- Kopyanın scriptlerini çakışmasın diye siliyoruz
                end
            end
            FakeBody.Parent = workspace
            FakeBody:PivotTo(Character:GetPivot())
            
            -- 2. Ana Karakterimizi Yerel Ekranımızda Gizliyoruz
        ...
        -- 3. Heartbeat ile Ağ Senkronizasyonunu Manipüle Ediyoruz
        ...
        -- --- DESYNC KAPATMA VE GERİ DÖNME KISMI ---
        if DesyncConnection then
            DesyncConnection:Disconnect()
            DesyncConnection = nil
        end
        
        -- Bıraktığımız Sahte Bedeni Temizliyoruz
        if FakeBody then
            FakeBody:Destroy()
            FakeBody = nil
        end
        
        -- Karakterimizi Tekrar Görünür ve Senkronize Yapıyoruz
        if Character then
            for _, part in pairs(Character:GetDescendants()) do
            ...
            -- Pozisyonu Tam Olarak İstemciye Göre Eşitliyoruz
...
-- FACE ATTACH (EN YAKIN OYUNCUNUN ENSESİNE YAPIŞMA SİSTEMİ)
local FaceAttachConnection = nil
local TargetPlayer = nil

-- En yakındaki canlı (Canı > 0) oyuncuyu bulma fonksiyonu
...
local function ToggleFaceAttach(state)
    if state then
        if FaceAttachConnection then FaceAttachConnection:Disconnect() end
        
        -- Her RenderStep (ekran yenilenmesinde) karakteri hedefin arkasına sabitliyoruz
        FaceAttachConnection = RunService.RenderStepped:Connect(function()
            if not Character or not Character:FindFirstChild("HumanoidRootPart") then return end
            
            -- Eğer hedef yoksa veya hedef öldüyse/oyundan çıktıysa yeni birini bul
            if not TargetPlayer or not TargetPlayer:FindFirstChild("HumanoidRootPart") or not TargetPlayer:FindFirstChildOfClass("Humanoid") or TargetPlayer:FindFirstChildOfClass("Humanoid").Health <= 0 then
                TargetPlayer = GetClosestPlayer()
            end
            
            -- Hedef kilitlendiyse arkasına yapış (Elimizi klavyeden çeksek bile çalışır)
            if TargetPlayer and TargetPlayer:FindFirstChild("HumanoidRootPart") then
                local TargetRoot = TargetPlayer.HumanoidRootPart
                
                -- Hedefin tam arkasında (Z ekseninde -2 blok geri, Y ekseninde azıcık havada) duracak CFrame hesaplama
                local OffsetCFrame = TargetRoot.CFrame * CFrame.new(0, 0.5, 2) 
                
                -- Karakterimizi hedefe doğru baktırarak arkasına ışınlıyoruz
                Character:PivotTo(CFrame.new(OffsetCFrame.Position, TargetRoot.Position))
                
                -- Düşerken veya yürürken titremeyi önlemek için hızı sıfırlıyoruz
                Character.HumanoidRootPart.Velocity = Vector3.new(0, 0, 0)
            end
        end)
    else
        -- Toggle kapatıldığında bağı kopar ve temizle
        if FaceAttachConnection then
            FaceAttachConnection:Disconnect()
            FaceAttachConnection = nil
        end
        TargetPlayer = nil
    end
end
...
-- TEK TOGGLE ÜZERİNDEN GELİŞMİŞ DESYNC GÖRÜNMEZLİK
chGroupBox:AddToggle("DesyncInvisibleToggle", {
    Text = "Desync Görünmezlik (Invisible)",
    Default = false,
    Callback = function(State)
        ToggleDesyncInvisible(State)
    end
})

-- FACE ATTACH OTO TAKİP TOGGLE ELEMENTİ
chGroupBox:AddToggle("FaceAttachToggle", {
    Text = "Face Attach (Oto Takip)",
    Default = false,
    Callback = function(State)
        ToggleFaceAttach(State)
    end
})
