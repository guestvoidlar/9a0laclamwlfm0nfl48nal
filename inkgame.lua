...
-- RLGL God Mode (PivotTo tabanlı ve rotasyonu bozmayan dikey ışınlanma)
...
local function ToggleGodMode(state)
    ...
    if Character then
        ...
        if state then
            -- Karakterin o anki açısını bozmadan Y eksenine 162 ekler
            Character:PivotTo(currentPivot * CFrame.new(0, 162, 0))
        else
            -- Kapatıldığında yine açıyı koruyarak dikey eksende başladığı hizaya geri indirir
            Character:PivotTo(currentPivot * CFrame.new(0, -162, 0))
        end
    end
end
...
powersGroupBox:AddButton("Quicksilver (MYTHİC!!)", function()
    LocalPlayer:SetAttribute("_EquippedPower", "QUİCKSİLVER")
end)
...
-- PivotTo ile eklenen yeni God Mode Toggle'ı
...
RedLightMainBox:AddButton("Finish RDGL", function()
    if Character then
        Character:PivotTo(CFrame.new(Vector3.new(-215.82, 1023.05, 145.96)))
    end
end)
...
RedLightMainBox:AddButton("Remove Injury", function()
    ...
    for _, p in pairs(Char:GetChildren()) do
        if p:IsA("BasePart") and p:FindFirstChild("BoneCustom") then
            p.BoneCustom:Destroy()
        end
    end
...
hiderGroupBox:AddButton("Teleport 40 Blocks Down", function()
    if Character then
        Character:PivotTo(Character:GetPivot() + Vector3.new(0, -40, 0))
    end
end)
                ...
                for _, v in pairs(workspace:GetDescendants()) do
                    if v:IsA("BasePart") and v.Name:lower() == "glasspart" then
                        ...
                        if isFake and not v:FindFirstChild("AntiFallPlatform") then
                            ...
                            plate.Size = v.Size
                            plate.CFrame = v.CFrame + Vector3.new(0, 0.6, 0)
...
jrGroupBox:AddButton("TP to End", function()
    if Character then
        Character:PivotTo(CFrame.new(Vector3.new(720.83, 202.7, 921.26)))
    end
end)
